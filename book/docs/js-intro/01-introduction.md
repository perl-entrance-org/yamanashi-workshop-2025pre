...
一般的に`console.log`に該当する処理は、他の言語では`print`と呼ぶことが多いため、`console.log`を多用して情報を表示し、デバッグ(プログラムのバグを見つけること)をprintデバッグなどと呼んだりします。
...
JavaScriptでは変数の宣言に`const`または`let`というキーワードを使います。
...
ただしまだ宣言していない変数を表示しようとするとエラーになります。
